# mixture null gene for qq plot for sim_obj across different K bin counts; can be run on local machine
rm(list = ls()) 
library(pbapply)
library(pbmcapply)
source("lpc_revisions_simulation_main_5_8_2026_final.R")
...
saveRDS(qq_by_K, file.path(sens_dir,"mixture_Ktest_pvecs_scaled_NB.rds"))
qq_by_K = readRDS(file.path(sens_dir, "mixture_Ktest_pvecs_scaled_NB.rds"))
type_i_by_K <- do.call(rbind, lapply(qq_by_K, empirical_reject_rate))
saveRDS(type_i_by_K, file.path(sens_dir, "mixture_pvecs_table.rds"))
...
ggplot2::ggsave(
  file.path(sens_dir,"mixture_qq_Ktest_scaled_NB.pdf"),
  qq_scaled_grid,
  width = 15,
  height = 10,
  dpi = 300

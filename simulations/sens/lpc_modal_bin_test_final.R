...
K = ask_K("Bin resolution: FD or a bin count", "FD") # if press enter will default to Freedman-Diaconis
...
scaled_metrics_file = if (use_discrete) {
  sprintf(
    "K_%s_NB_modal_sef_scaled_modal_discrete_metrics_list.RDS",
    K_label
  )
} else {
  sprintf(
    "K_%s_NB_modal_sef_scaled_modal_metrics_list.RDS",
    K_label
  )
}
...
message(sprintf("beginning modality-only scaled SEF tests using %s carrier with K = %s",
  carrier_type,
  K_label
))
...
scaled_metrics_list = run_preloaded_set(
  ...
  sprintf("scaled modality SEF (K = %s)", K_label)
)
